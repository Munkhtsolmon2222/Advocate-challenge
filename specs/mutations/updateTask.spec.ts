...
describe("updateTask Mutation", () => {
	...
	it("should throw an error if taskName is not unique", async () => {
		const task = {
			_id: "task123",
			userId: "user123",
			toString: () => "user123",
		...
		(Task.findOne as jest.Mock).mockResolvedValue({ _id: "anotherTask" });

		const input = { taskName: "Duplicate Task" };
		...
		).rejects.toThrow("taskName must be unique for each user");
	});
